...
function Diff(){
    ...
    function mkDiff(A,B,m,n,prec,matrix){
        if(m>0 && n>0 && A[m-1] === B[n-1]){
            ...
            //console.log(m,n,matrix[m][n-1],matrix[m-1][n],B[n-1]);
            buildDiff(B,n,n,prec,'+');
            //console.log(n-1,'+',B[n-1]);
        }
        else if(m>0 && (n === 0 || matrix[m][n-1] < matrix[m-1][n])){
            mkDiff(A,B,m-1,n,m-1,matrix);
            //console.log(m,n,matrix[m][n-1],matrix[m-1][n],A[m-1]);
            buildDiff(A,m,m,prec,'-');
            //console.log(m-1,'-',A[m-1]);
        }
    }

    function reverseDiff(diffData){
        let actualDiffData = [];
        for(let index = diffData.length - 3; index >= 0; index -= 3){
            actualDiffData.push(diffData[index]);
            actualDiffData.push(diffData[index+1]);
            actualDiffData.push(diffData[index+2]);
        }
        return actualDiffData;
    }
