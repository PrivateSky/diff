...
function RsyncDiff() {
    ...
    function processBlocks(srcString, blockSize, targetString){
        ...
        for(let i = 0 ; i < blocks.length; i++){
            storeMatches(blocks[i],i*blockSize,targetString,srcSet,targetSet);
        }
    ...
    function createDiff(srcString, blockSize, targetString, srcSet, targetSet) {
        ...
        if(targetString.length < indexPair[indexPair.length - 1].targetIndex + blockSize){
            blockSize = targetString.length - indexPair[indexPair.length - 1].targetIndex - 1;
        }
        if (targetString.length - indexPair[indexPair.length - 1].targetIndex - blockSize > 0) {
            ...
            if(preSubtractionLastIndex + blockSize <= srcString.length) {
                ...
                diffArray.push(preSubtractionLastIndex + (srcString.length - preSubtractionLastIndex) + toAdd);
