...
function RsyncDiff() {
    ...
    function storeMatches(srcBlock, blockIndex, targetString, srcSet, targetSet, lastBlockSize) {
        ...
        if(lastIndexTarget === undefined){
            ...
            if(initial + blockSize > targetString.length){
                return;
            }
        }
        windowCheckSum = easyCheckSum(targetString.slice(initial,initial + blockSize));
        for (let i = initial + blockSize; i < targetString.length + 1; i++) {
